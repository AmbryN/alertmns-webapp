...
import { loadNotificationsFailure, loadNotificationsSuccess, markNotificationsAsSeen, markNotificationsAsSeenFailure, markNotificationsAsSeenSuccess } from './notification.action';
    ...
    this.actions$.pipe(
      ...
      switchMap((action) =>
        this.notificationService.markNotificationsAsSeen(action.channelId).pipe(
          map((notifications) => markNotificationsAsSeenSuccess({ notifications: notifications })),
          catchError((error) => of(markNotificationsAsSeenFailure({ error: error })))
        )
      )
    )
  );
